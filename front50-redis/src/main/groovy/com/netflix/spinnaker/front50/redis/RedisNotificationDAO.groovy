...
class RedisNotificationDAO implements NotificationDAO {
  ...
  RedisTemplate<String, Notification> template
  ...
  Notification get(HierarchicalLevel level, String name) {
    return template.opsForValue().get(key(name)) ?: [email: []] as Notification // an empty Notification is expected for applications that do not exist
  }
  ...
  void save(HierarchicalLevel level, String name, Notification notification) {
    ...
    template.opsForValue().set(key(name), notification)
  }

  @Override
  void delete(HierarchicalLevel level, String name) {
    template.delete(key(name))
  }
